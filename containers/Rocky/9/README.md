# OCS Rocky Linux 9 Container

Open Cluster Scheduler on Rocky Linux 9 (latest patch release via the
floating `rockylinux/rockylinux:9` tag) — single node or a 3-node Docker
Compose cluster. OCS installs automatically on first start.

## Single Node

```bash
docker build -t ocs-rocky:9 .
docker run -it --rm ocs-rocky:9 /bin/bash
```

Pin a version (default is the latest supported):

```bash
docker run -it --rm -e OCS_VERSION=9.0.12 ocs-rocky:9 /bin/bash
```
...
This cluster uses the same container names (`ocs-master`, `ocs-worker1/2`),
network name and subnet (`10.100.0.0/16`) as the openSUSE cluster in
`containers/openSUSE/15.6/`, so only one of the two can run on a host at a
time. `docker compose down` the other one first.
...
docker compose build --pull         # refresh to the newest Rocky 9.x patch image
```
...
The first start needs internet access to the Rocky mirrors (BaseOS,
AppStream, CRB) and EPEL: `ocs.sh` installs its own dependencies with dnf,
enables the CRB repository for `libtirpc-devel` and pulls `screen` from
EPEL. A failing EPEL only costs `screen` (warning in the log); failing
CRB/BaseOS mirrors break the installation.
...
All files except the two Dockerfiles and this README are identical copies of
their counterparts in `containers/openSUSE/15.6/` — change them there and
here together (see the repository `CLAUDE.md`).
