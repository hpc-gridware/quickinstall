...
# sourced from the .bashrc of root and gridware: not every distro reads
# /etc/profile.d for the interactive non-login shell that
# "docker exec -it ocs-master bash" creates (openSUSE does not), so both
# paths are needed. Double sourcing is harmless: settings.sh is guarded by
# SGE_ROOT and the banner by _ocs_banner_shown.
#
